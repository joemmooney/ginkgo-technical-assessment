...
def findProteinMatch(dna_string, instance):
    try:
...
class SearchProteinsView(viewsets.ModelViewSet):
    ...
    def create(self, request):
        serializer = SearchProteinsSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            async_task(findProteinMatch, request.data['dnasequence'], serializer.instance)
